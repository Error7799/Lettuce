...
/* ── Presets ──────────────────────────────────────────────────────────────
 * SillyTavern's model: a preset is a *named bundle* you switch between, and
 * it owns both halves of a generation setup — the sampler values and the
 * ordered prompt blocks. LettuceAI already stores prompt blocks as a
 * SystemPromptTemplate, so a preset points at one rather than duplicating it;
 * switching preset re-points the chat at that template and applies the
 * sampler values on top of the model.
 *
 * Presets live inside AppState, which the Rust side persists as an opaque
...
export const AppStateSchema = z.object({
  ...
  /**
   * Bionic reading intensity for message text — the ADHD Reader.
