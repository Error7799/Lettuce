...
export async function withAppState(mutator: (draft: AppState) => void): Promise<AppState> {
...
export async function setAdhdReading(intensity: AppState["adhdReading"]): Promise<void> {
  await withAppState((state) => {
    state.adhdReading = intensity;
  });
}
