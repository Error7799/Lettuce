...
export function DiscoveryCardDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadCard = async () => {
      ...
      try {
        // Character Tavern's endpoint returns a richer envelope (alternate
        // greetings, author info), so it keeps its own path; other catalogues
        // go through the provider.
        const response =
          provider.id === "character-tavern"
            ? await fetchCardDetail(decodedPath)
            : ({ card: await provider.detail(decodedPath) } as Awaited<
                ReturnType<typeof fetchCardDetail>
              >);
        setCardData(response);

        // Load image
        if (response.card.path) {
          const direct = provider.imageUrl(response.card as never);
          if (direct && provider.id !== "character-tavern") {
            setImageUrl(direct);
          } else {
  ...
  const handleDownload = async () => {
    ...
    try {
      // Each catalogue stores definitions differently, so the provider owns
      // the import. Character Tavern's still runs through the Rust importer.
      const characterId =
        provider.id === "character-tavern"
          ? await importCharacter(cardData.card.path)
          : await provider.importCard(cardData.card.path);
