...
export function DiscoveryPage() {
  ...
  const loadSections = useCallback(
    async (isRefresh = false) => {
      try {
        ...
        // Character Tavern keeps its purpose-built sections endpoint; other
        // catalogues have no such thing, so their three rows come from three
        // browse calls with different orderings.
        const data =
          provider.id === "character-tavern"
  ...
  // Which catalogue the results come from. Sources needing an account are
  // shown but disabled, so it is clear they exist and why they are unavailable.
  const sourceBar = (
    <div className="scrollbar-hide flex gap-2 overflow-x-auto">
      {providers.map((entry) => {
        const isActive = entry.id === providerId;
        return (
          <button
            key={entry.id}
            type="button"
            onClick={() => setProviderId(entry.id)}
            disabled={entry.requiresAuth}
            title={entry.requiresAuth ? "Needs an account on that site" : entry.description}
            className={cn(
              "shrink-0 rounded-full border px-3 py-1.5 text-xs font-medium transition-all",
              "disabled:cursor-not-allowed disabled:opacity-40",
              isActive
                ? "border-accent/40 bg-accent/20 text-accent"
                : "border-fg/10 bg-fg/5 text-fg/55 hover:border-fg/20 hover:bg-fg/10 hover:text-fg",
            )}
          >
            {entry.label}
          </button>
      ...
      >
        {isMobileViewport ? (
          <div className="sticky top-0 z-20 bg-surface/95 backdrop-blur-md">
            ...
            <div className="px-4 pb-3">{sourceBar}</div>
            ...
            filters={
              searching ? undefined : (
                <div className="flex flex-col gap-2">
                  <div className="flex gap-2">{sourceBar}</div>
                  <div className="flex gap-2">{tabBar}</div>
                </div>
              )
            }
          />
