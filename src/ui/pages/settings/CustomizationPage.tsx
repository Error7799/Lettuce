...
import { setAdhdReading } from "../../../core/storage/appState";
...
export function CustomizationPage() {
  ...
  const [adhdIntensity, setAdhdIntensity] = useState<BionicIntensity>("off");
  ...
  useEffect(() => {
    ...
    void getAppState()
      .then((state) => {
        if (!cancelled) setAdhdIntensity(state.adhdReading ?? "off");
      })
      .catch(() => {
        // Leave the default; the control still works once changed.
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <div className="flex h-full flex-col">
      <section className="flex-1 overflow-y-auto px-3 pt-3 pb-6 space-y-6">
        {/* ADHD Reader. Literal strings rather than t() keys: adding them would
            mean editing every locale file, and an untranslated key reads worse
            than plain English. */}
        <div>
          ...
          <div className={cn("rounded-xl border px-4 py-3", "border-fg/10 bg-fg/5")}>
            <div className="text-sm font-medium text-fg">ADHD Reader</div>
            <div className="mt-0.5 text-[11px] leading-relaxed text-fg/45">
              Bolds the first few letters of each word so your eye can latch onto the stem. Higher
              settings bold more.
            </div>
            <div className="mt-3 flex gap-1.5">
              {(["off", "light", "medium", "strong"] as const).map((level) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setAdhdIntensity(level);
                    void setAdhdReading(level);
                  }}
                  className={cn(
                    "flex-1 rounded-lg border px-2 py-2 text-xs font-medium capitalize",
            ...
            {adhdIntensity !== "off" && (
              <p className="mt-3 rounded-lg border border-fg/10 bg-fg/5 px-3 py-2 text-xs leading-relaxed text-fg/60">
                <b className="font-bold text-fg/90">Pre</b>vi<b className="font-bold text-fg/90">ew</b>{" "}
                — open a chat to see it applied to real messages.
              </p>
            )}
          </div>
        </div>
