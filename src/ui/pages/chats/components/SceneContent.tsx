...
import { useAdhdReading } from "../../../hooks/useAdhdReading";
...
export function SceneContent({ content, className, onImageClick, textColors }: SceneContentProps) {
  ...
  // Bionic reading is applied as a pass over the rendered output, so it works
  // for every markdown construct without the renderer knowing about it.
  const adhdIntensity = useAdhdReading();

  if (segments.length === 1 && segments[0].type === "text") {
    return (
      <AdhdReader intensity={adhdIntensity} contentKey={content}>
        <MarkdownRenderer
          content={content}
          className={className}
